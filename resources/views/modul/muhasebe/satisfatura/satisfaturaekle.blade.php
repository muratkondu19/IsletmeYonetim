    ...
    <section class="content">
        ...
        <div class="box box-danger">
            <div class="box-header">
                <h3 class="box-title">Yeni Satış Faturası Ekle</h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
                @if (session('success'))
                ...
                @endif
                <form action="{{ route('muhasebe.satisfaturaStore') }}" method="POST" role="form"
                    ...
                    <div class="form-vertical col-md-6 master">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control"  value="{{ $data[0]['satisSiparisBelgeNo'] }}" name="satisfaturaBelgeNumara"
                        ...
                        <div class="form-group">
                            ...
                            <select name="satisfaturaHesapKod" class="form-control select2 carikod">
                                ...
                                @foreach (\App\Models\Muhasebe\Cari::all() as $k => $v)
                                    <option @if ($v['id'] == $data[0]['satisSiparisCariId']) selected @endif value="{{ $v['cariMHK'] }}">
                        ...
                        <div class="form-group">
                            ...
                            <select name="satisfaturaHesapAd" class="form-control cariad select2">
                                ...
                                @foreach (\App\Models\Muhasebe\Cari::all() as $k => $v)
                                    <option @if ($v['id'] == $data[0]['satisSiparisCariId']) selected @endif  value="{{ $v['cariAd'] }}">
                                        {{ \App\Models\Muhasebe\Cari::getCariAd($v['id']) }} </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <input type="date" value="{{ date('Y-m-d') }}" class="form-control"
                                name="satisfaturaBelgeTarih">
                        </div>
                        <div class="form-group">
                            <label>Açıklama</label>
                            <input type="text" class="form-control"
                                value="{{ $data[0]['satisSiparisAciklama'] }}'e ait satış faturası kaydıdır."
                                name="satisfaturaAciklama" placeholder="Açıklama giriniz.">
                        </div>
                    </div>
                                ...
                                @foreach ($dataIslem as $k => $v)
                                    <tr class="islem_field">
                                        ...
                                        <td><select class="form-control"
                                                ...
                                                <option @if ($v['satisSiparisBirim'] == 'KG') selected @endif value="KG">KG</option>
                                            </select></td>
                                        <td><input type"text" class="form-control" id="miktar" value="{{ $v['satisSiparisMiktar'] }}" placeholder="Miktar" name="islem[{{ $k }}][satisMiktar]"> </td>
                                        <td><input type"text" class="form-control" id="birimfiyat" value="{{ $v['satisSiparisBirimFiyat'] }}" placeholder="Birim Fiyatı" name="islem[{{ $k }}][satisBirimFiyat]"> </td>
                                        <td><input type"text" class="form-control" id="toplamtutar" value="{{ $v['satisSiparisToplamTutar'] }}" placeholder="Toplam Tutar"name="islem[{{ $k }}][satisToplamTutar]"> </td>
                                        <td><button id="remove_buton" type="button" class="btn btn-danger btn-sm">X</button>
                                        </td>
                                @endforeach
                            </tbody>
                    ...
                    <div class="box-footer pull-right box-tools">
                        <button type="submit" class="btn btn-success">Satış Faturası Kaydet</button>
                    </div>
                </form>
            </div>
            <!-- /.box-body -->
    ...
    <script>
        ...
        $("#addRowBtn").click(function() {
            ...
            <?php foreach(\App\ Models\ Envanter\ Depo::all() as $key => $value) {?>
                newRow += '<option data-depoad=<?php echo $value['depoKod']; ?> value="<?php echo $value['depoAd']; ?>"><?php echo $value['depoAd']; ?></option>';
                <?php } ?>
            ...
            '<td><select class="form-control select2 depokod" name="islem[' + i + '][satisDepoKod]">' +
                '<option value="0">Depo Seçiniz</option>';
            <?php foreach(\App\ Models\ Envanter\ Depo::all() as $key => $value) { ?>
                newRow += '<option data-depokod=<?php echo $value['depoAd ']; ?> value="<?php echo $value['depoKod']; ?>"><?php echo $value['depoKod']; ?></option>';
                <?php  } ?>

                newRow += '</select></td>' +
                '<td><select class="form-control select2 hesapad" name="islem[' + i + '][satisMuhasebeAd]">' +
                '<option value="0">Muhasebe Stok Hesap Adı</option>';
            ...
            newRow += '</select></td>' +
                ...
                '<option value="0">Muhasebe Stok Hesap Kodu</option>';
            <?php foreach(\App\ Models\ Envanter\ StokKart::all() as $key => $value) {?>
