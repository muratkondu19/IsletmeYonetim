    ...
    <section class="content">
        ...
        <div class="box box-danger">
            <div class="box-header">
                <h3 class="box-title">Ürün Ağacı İnceleme Düzenleme</h3>
            ...
            <div class="box-body">
                ...
                @endif
                <form action="{{  route('urun.urunagaciUpdate',['id'=>$data[0]['id']])  }}" method="POST" role="form"
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <select name="urunagaciFirmaKod" class="form-control select2">
                            ...
                            @foreach (\App\Models\User::all() as $k => $v)
                                <option  @if($v["id"]==$data[0]["urunagaciFirmaKod"]) selected @endif value="{{ $v['id'] }}">
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <select name="urunagaciStokKod" class="form-control select2">
                            ...
                            @foreach (\App\Models\Envanter\StokKart::all() as $k => $v)
                                <option @if($v["stokKod"]==$data[0]["urunagaciStokKod"]) selected @endif value="{{ $v['stokKod'] }}">
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <select name="urunagaciStokAd" class="form-control select2">
                            ...
                            @foreach (\App\Models\Envanter\StokKart::all() as $k => $v)
                                <option  @if($v["stokAd"]==$data[0]["urunagaciStokAd"]) selected @endif value="{{ $v['stokAd'] }}">
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <select name="urunagaciStokTakipBirim" class="form-control select2" style="width: 100%">
                            ...
                            <option @if($data[0]["urunagaciStokTakipBirim"]=="Adet") selected @endif value="Adet">Adet</option>
                            <option @if($data[0]["urunagaciStokTakipBirim"]=="Gram") selected @endif value="Gram">Gram</option>
                            <option @if($data[0]["urunagaciStokTakipBirim"]=="Litre") selected @endif value="Litre">Litre</option>
                            <option @if($data[0]["urunagaciStokTakipBirim"]=="Kilogram") selected @endif value="Kilogram">Kilogram</option>
                        </select>
                    </div>
                    <div class="form-group col-md-6">
                        ...
                        <select name="urunagaciStokGirişDepoAd" class="form-control select2">
                            ...
                            @foreach (\App\Models\Envanter\Depo::all() as $k => $v)
                                <option  @if($v["depoAd"]==$data[0]["urunagaciStokGirişDepoAd"]) selected @endif value="{{ $v['depoAd'] }}">
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <input type="text" class="form-control" name="urunagaciAciklama" value=" {{$data[0]['urunagaciAciklama']}}">
                    </div>
                    ...
                    <div class="form-group col-md-12">
                        ...
                        <table id="urunagaciFirma" class="table table-striped">
                            ...
                            <tbody>
                                ...
                                <td><select class="form-control" name="islem[{{$k}}][urunagacimalzemeSatırTip]">
                                <option selected value="">Satır Tipi</option>
                                <option @if($v["urunagacimalzemeSatırTip"]=="Hammadde") selected @endif  value="Hammadde">Hammadde</option>
                                <option @if($v["urunagacimalzemeSatırTip"]=="Yarı Mamül") selected @endif  value="Yarı Mamül">Yarı Mamül</option>
                                <option @if($v["urunagacimalzemeSatırTip"]=="Mamül") selected @endif  value="Mamül">Mamül</option>
                            
                                </select></td>
                                <td><select class="form-control stokkod" name="islem[{{$k}}][urunagacimalzemeStokKod]">' +
                                <option value="0">Stok Kodu</option>
                                @foreach (\App\Models\Envanter\StokKart::all() as $key => $value) 
                                 <option @if($value['id']==$v['urunagacimalzemeStokKod']) selected @endif  data-stokbirim={{$value["stokBirim"]}} data-stokkod= {{$value["id"]}}  value=" {{$value['id']}}">{{ $value['stokKod'] }} / {{$value['stokAd'] }}</option>
                                 @endforeach
                                    
                                 </select></td>
                                <td><select class="form-control stokad" name="islem[{{$k}}][urunagacimalzemeStokAd]">
                                <option value="0">Stok Adı</option>
                                 @foreach (\App\Models\Envanter\StokKart::all() as $key => $value) 
                                 <option @if($value['id']==$v['urunagacimalzemeStokAd']) selected @endif data-stokbirim={{$value["stokBirim"] }} data-stokad={{$value["id"] }} value="{{$value['id']}}">{{$value['stokAd'] }} {{ $value['stokKod'] }}</option>
                                 @endforeach
                        
                                </select></td>
                                    <td><select class="form-control" name="islem[{{$k}}][urunagacimalzemeOperasyonAd]">
                                    <option value="0">Operasyon Adı</option>
                                    @foreach (\App\Models\Urun\Operasyon::all() as $key => $value) 
                                    <option  @if($value['id']==$v['urunagacimalzemeOperasyonAd']) selected @endif value="{{$value['id']}}">{{$value['operasyonAd']}}</option>
                                    @endforeach

                                </select></td><td><select class="form-control stokbirim" name="islem[{{$k}}][urunagacimalzemeBirim]">
                                    <option selected value="">Birim Seçiniz</option>
                                    <option @if($v["urunagacimalzemeBirim"]=="Adet") selected @endif value="Adet">Adet</option>
                                    <option @if($v["urunagacimalzemeBirim"]=="Litre") selected @endif value="Litre">Litre</option>
                                    <option  @if($v["urunagacimalzemeBirim"]=="KG") selected @endif value="KG">KG</option>
                    
                                </select></td>
                                    <td><input type"text" class="form-control" id="miktar" value="{{$v['urunagacimalzemeMiktar']}}" name="islem[{{$k}}][urunagacimalzemeMiktar]"> </td>
                    ...
                    <div class="box-footer pull-right box-tools">
                        <button type="submit" class="btn btn-success">Ürün Ağacı Güncelle</button>
                    </div>
                </form>
            </div>
            <!-- /.box-body -->
